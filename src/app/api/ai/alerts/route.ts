...
export async function GET(request: NextRequest) {
  try {
    ...
    const alerts = await generateSpendingAlerts(financialData)

    return NextResponse.json({ alerts, month })
  } catch (error) {
    console.error('Alerts error:', error)
    return NextResponse.json({ alerts: [], error: 'Failed to generate alerts' })
  }
}
